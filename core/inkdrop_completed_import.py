...
# Shared with inkdrop_web_config's copies -- see inkdrop_manga_unit_policy.
...
def convert_pdf_to_cbz(source, dest, target, issue_row=None, dpi=160):
    ...
    with tempfile.TemporaryDirectory(prefix="kavita-import-pdf-") as tmp:
        ...
        comicinfo = (
            ...
            f"{xml_node('Publisher', publisher)}"
...
def _existing_file_rejected_by_artifact_gate(path, dest_path, event, decision, conn, digest, target, dry_run, dest_label):
    """React to an artifact_acceptance_decision rejection for an
    existing-file match -- shared by the DB-digest and filesystem-probe
    existing-file paths in `import_files()`, which differ only in which
    path matched (`dest_path`) and how the manual-review note describes
    that match (`dest_label`). Only called from within `if kind == "comics"
    and target:`, so `target` is always truthy here, matching both original
    call sites. Returns True if the caller should treat this candidate as
    skipped (append to `skipped` and `continue`).
    """
    if decision.get("completion_eligible"):
        return False
...
def _flush_pending_write_before_slow_verification(conn):
    """Commit any already-pending write before purely-filesystem verification
    work that needs no lock of its own -- comic archive read/testzip, page
    decode for artifact acceptance.

    import_files() keeps one connection open across its entire multi-file
    pass. Python's sqlite3 module opens a transaction implicitly on the first
    write and does not close it until the next commit()/rollback() -- one
    file's bookkeeping write can still be uncommitted when the very next
    step is an expensive, non-database archive check. Confirmed live: a
    write left open across an unrelated sleep() blocks a separate
    connection's BEGIN IMMEDIATE with "database is locked" for the entire
    sleep, even though nothing in that window touched the database at all.
    That's how one write ends up holding SQLite's write lock for as long as
    the archive/page-decode work takes, not just the moment the write itself
    ran -- measured live at 41% of wall-clock blocked on 2026-08-01.
    Committing here has no effect on what gets written or when it becomes
    durable; it only closes the lock window sooner than the next
    naturally-occurring commit() in the loop would have.
    """
            ...
            if existing:
                ...
                if existing_dest.exists() and (not target or existing_dest.parent == target_dir):
                    ...
                    if kind == "comics" and target:
                        ...
                        if _existing_file_rejected_by_artifact_gate(
                            path, existing_dest, event, decision, conn, digest, target, dry_run, "imported-file"
                        ):
                            skipped.append(event)
                            continue
                    log(event)
                ...
                if same_file:
                    ...
                    if kind == "comics" and target:
                        ...
                        if not decision.get("completion_eligible") and human_override(
                            ...
                            decision = {**decision, "completion_eligible": True}
                        if _existing_file_rejected_by_artifact_gate(
                            path, same_file, event, decision, conn, digest, target, dry_run, "same-file"
                        ):
                            skipped.append(event)
                            continue
                    conn.execute(
