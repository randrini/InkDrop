...
# The autopilot calls this script once per monitored series, not once per
# pass -- with enough monitored series that's a fresh HTTP request to the
# same shared feed URL every few seconds. GetComics answers that with a 429,
# which used to trip the (correctly bounded) failure backoff below and stall
# every series checked during that window, even though the feed itself was
# never actually unhealthy. Reusing a very recently fetched copy avoids
# tripping the rate limit in the first place, independent of that backoff.
...
def fetch_feed(cache, args):
    ...
    last_fetch_at = float(feed_state.get("last_fetch_at") or 0)
    cached_xml = feed_state.get("last_xml")
    if not args.force_feed and cached_xml and (now - last_fetch_at) < MIN_POLL_INTERVAL_SECONDS:
        return cached_xml, "cached_recent"
