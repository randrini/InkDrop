...
def queued_transfer_stale_seconds(transfer, stall_policy=None):
    """Resolve the stale-after window for a transfer sitting in SLSKD's queue.

    This used to drop to a 45-minute window whenever the same peer had
    another transfer of the same candidate already moving bytes -- on the
    theory that a busy peer elsewhere meant this queued copy was a lost
    cause. That reasoning ran backwards: a peer actively sending us the same
    candidate somewhere else is evidence the transfer is alive, not proof
    this queued copy is dead, and it gave the whole queued-wait policy no
    chance to apply. Every queued transfer now gets the same patient window
    (48h default, configurable via
    automation.queue_watchdog_slskd_queued_wait_hours) regardless of what
    else the peer happens to be sending us.
    """
