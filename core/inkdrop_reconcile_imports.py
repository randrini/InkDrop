...
def normalize_download_path(path_value):
    # Was called (SAB history display's ambiguous-multi-archive fallback) but
    # never defined -- guaranteed NameError on any call. Same client-reported
    # path is already resolved through download_client_host_path() one call
    # earlier at that call site (to build archive_paths); this must resolve
    # it the same way rather than display the raw, unmapped client path.
    return download_client_host_path(path_value)
...
def prefixed_metadata_identity(provider, metadata_id, fallback=None):
    ...
    if provider in {"comicvine", "mangadex"} and metadata_id:
        if inkdrop_state is not None and hasattr(inkdrop_state, "metadata_identity_format"):
            return inkdrop_state.metadata_identity_format(provider, metadata_id)
