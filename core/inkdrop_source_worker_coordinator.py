...
from core import inkdrop_missing_recovery_policy
    ...
    trusted_identity = inkdrop_manual_search.trusted_target_unit_identity(
        {
            ...
            # Absent for callers whose queue dict doesn't carry series_raw_json
            # yet -- series_manga_unit_override() returns "" in that case,
            # which is exactly "no override", so this can't regress any caller.
            "series_manga_unit_override": inkdrop_manga_unit_policy.series_manga_unit_override(
                {"raw_json": queue.get("series_raw_json")}
            ),
        }
    )
    ...
    *,
    ...
    for task in tasks:
        ...
        if source_memory_db_path:
            from core import inkdrop_source_suppression as suppression
