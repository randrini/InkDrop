...
try:
    from core import inkdrop_source_worker_cli
except Exception:
...
def _provider_config_summary(args):
    return {
        "prowlarr_provider": {
            "enabled": bool(getattr(args, "prowlarr_provider_enabled", not getattr(args, "skip_prowlarr", False))),
            "settings_source": getattr(args, "prowlarr_provider_settings_source", "fallback"),
...
def run_provider_then_companion_maintenance(args, provider_runner=None, maintenance_runner=None):
    ...
    try:
        ...
        # Each was 1 before: with ~28 monitored manga series never companion-linked
        # at all and reconcile only running once per autopilot pass, that backlog
        # was never going to clear. 3 is a modest multiple, not "unbounded" --
        # both are still hard-capped downstream (reconcile at 5, refresh at 5).
        maintenance_runner(reconcile_limit=3, refresh_limit=3)
    except Exception as exc:
