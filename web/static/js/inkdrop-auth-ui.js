(function () {
  ...
  function principalIsAdministrator(principal = state.principal) {
    const role = String(principal?.user?.role || principal?.role || "")
      .trim()
      .toLowerCase();
  ...
  function publishAuthorizationState() {
    ...
    document.body.dataset.inkdropAdministrator = administrator
      ? "true"
      : "false";
    window.dispatchEvent(
      new CustomEvent("inkdrop-auth-state", { detail: { administrator } }),
    );
  }
  ...
  function applyAuthArtworkPreference(host) {
    ...
    if (allowed && !host.dataset.authArtVariant)
      host.dataset.authArtVariant = authArtworkVariant();
  ...
  function applyPasswordPolicy(form) {
    ...
    form
      .querySelectorAll(
        'input[type="password"][name="password"], input[type="password"][name="new_password"], input[type="password"][name="confirm"], input[type="password"][name="confirm_password"]',
      )
      .forEach((input) => {
    ...
    if (target) {
      const limits = policy.minimum
        ? `At least ${policy.minimum} characters`
        : "Use a strong password";
      const maximum = policy.maximum
        ? ` and no more than ${policy.maximum}`
        : "";
      const freedoms = !policy.compositionRequired
        ? " No special character or composition rule is required."
        : "";
  ...
  function renderBootstrap() {
    const form = authCard(
      ...
      "Set up the account that will manage this InkDrop instance.",
      `<form class="inkdrop-auth-form">${field("Username", "text", "username", "username")}${field("Password", "password", "password", "new-password")}${field("Confirm password", "password", "confirm", "new-password")}<p class="inkdrop-auth-secondary" data-password-policy></p><div class="inkdrop-auth-message" data-auth-message role="status"></div><button type="submit">Create administrator</button></form>`,
    );
    ...
    form.addEventListener("submit", async (event) => {
      ...
      if (password !== form.elements.confirm.value)
        return message(form, "Passwords do not match.", "bad");
  ...
  function renderLogin(reason) {
    ...
    if (!builtIn.enabled && external.enabled) {
      const form = authCard(
        ...
        external.ready
          ? "Sign in through the configured identity proxy, then reload InkDrop."
          : external.next_action || "External authentication is not ready.",
        `<form class="inkdrop-auth-form"><div class="inkdrop-auth-message" data-auth-message role="status"></div><button type="submit">Check session</button></form>`,
      );
    ...
    const form = authCard(
      "Sign in to InkDrop",
      reason || "Use your InkDrop administrator account.",
      `<form class="inkdrop-auth-form">${field("Username", "text", "username", "username")}${field("Password", "password", "password", "current-password")}<div class="inkdrop-auth-message" data-auth-message role="status"></div><button type="submit">Sign in</button>${external.enabled ? '<p class="inkdrop-auth-secondary">External authentication is also enabled.</p>' : ""}<p class="inkdrop-auth-secondary">Locked out? Recovering access needs local access to this server -- see the password recovery section of the install documentation.</p></form>`,
    );
    form.addEventListener("submit", async (event) => {
      event.preventDefault();
      const submit = form.querySelector("button[type=submit]");
  ...
  function renderFatal(text) {
    const form = authCard(
      "InkDrop authentication unavailable",
      text || "The authentication service did not respond.",
      `<form class="inkdrop-auth-form"><button type="submit">Retry</button></form>`,
    );
    form.addEventListener("submit", (event) => {
      event.preventDefault();
      start();
    ...
    dialog
      ...
      .forEach((button) =>
        button.addEventListener("click", () =>
          renderAccountPanel(button.dataset.tab),
        ),
      );
  ...
  async function openAccountDialog() {
    ...
    dialog.querySelector("[data-auth-mode]").textContent =
      `Mode: ${(state.status?.mode || "unknown").replaceAll("_", " ")}`;
  ...
  function closeAccountDialog() {
    ...
    if (dialog) {
      dialog.querySelector("[data-account-panel]").textContent = "";
      dialog.close();
    }
  }
  ...
  async function changePassword(event) {
    ...
    if (
      form.elements.new_password.value !== form.elements.confirm_password.value
    )
      return message(form, "New passwords do not match.", "bad");
    try {
      await request("/api/auth/password", {
        ...
        body: {
          ...
          revoke_other_sessions: form.elements.revoke_other_sessions.checked,
        },
      });
      form.reset();
  ...
  async function renderApiKeys(panel) {
    ...
    try {
      ...
      panel
        .querySelector(".inkdrop-api-key-form")
        .addEventListener("submit", createApiKey);
      panel
        .querySelectorAll("[data-revoke-key]")
        .forEach((button) =>
          button.addEventListener("click", () =>
            revokeApiKey(button.dataset.revokeKey, panel),
          ),
        );
    } catch (error) {
      panel.innerHTML = `<section class="inkdrop-account-section"><h2>API Keys</h2><p class="bad">${escapeHtml(error.message)}</p></section>`;
    }
  }
    ...
    panel
      ...
      .addEventListener("click", async () => {
        try {
          ...
          message(panel, "Key copied. Store it securely.", "good");
        } catch (_error) {
          message(
            panel,
            "Copy failed. Select the key manually before leaving this screen.",
            "bad",
          );
        }
      });
    panel.querySelector("[data-key-done]").addEventListener("click", () => {
      state.oneTimeKey = "";
      renderApiKeys(panel);
    });
  }
  ...
  async function revokeApiKey(id, panel) {
    if (
      !id ||
      !window.confirm(
        "Revoke this API key? Clients using it will stop working.",
      )
    )
      return;
    try {
      await request(`/api/auth/api-keys/${encodeURIComponent(id)}/revoke`, {
        method: "POST",
        body: {},
      });
      renderApiKeys(panel);
      ...
      panel
        .querySelector(".inkdrop-account-section")
        .insertAdjacentHTML(
          "afterbegin",
          `<p class="inkdrop-auth-message bad">${escapeHtml(error.message)}</p>`,
        );
    }
  }
  ...
  async function start() {
    ...
    host.hidden = false;
    host.innerHTML =
      '<section class="inkdrop-auth-card"><p>Checking InkDrop authentication…</p></section>';
    try {
      const payload = await request("/api/auth/status");
      state.status = payload.auth || payload;
      if (state.status.built_in_auth?.bootstrap_required)
        return renderBootstrap();
      if (!state.status.required && !state.status.setup_required)
        return revealApplication(null);
      await validateSession();
    } catch (error) {
  ...
  window.addEventListener("inkdrop:session-expired", () => {
    ...
    document.documentElement.classList.add("inkdrop-auth-pending");
    renderLogin(
      "Your session expired. Sign in again, then retry what you were doing.",
    );
  });
  document.addEventListener("DOMContentLoaded", start, { once: true });
  window.__inkdropSessionPollTimer = window.setInterval(() => {
    if (
      !window.__inkdropAuthReady ||
      document.hidden ||
      state.status?.required === false ||
      window.__inkdropAuthLoggedOut
    )
      return;
