(function () {
  ...
  function accountDialog() {
    ...
    dialog
      ...
      .forEach((button) => button.addEventListener("click", () => renderAccountPanel(button.dataset.tab)));
  ...
  async function renderApiKeys(panel) {
    ...
    try {
      ...
      panel
        ...
        .forEach((button) => button.addEventListener("click", () => revokeApiKey(button.dataset.revokeKey, panel)));
    } catch (error) {
  ...
  function showOneTimeKey(panel) {
    ...
    panel.querySelector("[data-copy-key]").addEventListener("click", async () => {
      try {
        ...
        message(panel, "Copy failed. Select the key manually before leaving this screen.", "bad");
      }
    });
  ...
  async function revokeApiKey(id, panel) {
    ...
    try {
      await request(`/api/auth/api-keys/${encodeURIComponent(id)}/revoke`, { method: "POST", body: {} });
      renderApiKeys(panel);
    } catch (error) {
      panel
        .querySelector(".inkdrop-account-section")
        .insertAdjacentHTML("afterbegin", `<p class="inkdrop-auth-message bad">${escapeHtml(error.message)}</p>`);
    }
  }
