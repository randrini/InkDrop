(function (global) {
  ...
  function field(labelText, control) {
    ...
    label.append(
      element("span", "inkdrop-table-preference-label", labelText),
      control,
    );
    return label;
  }
  ...
  function enhanceMenu(menu, root) {
    if (!menu || menu.dataset.inkdropMenuEnhanced === "true")
      return function () {};
    ...
    function closeOtherMenus() {
      if (!menu.open) return;
      root
        .querySelectorAll("details.arr-table-menu[open]")
  ...
  function enhanceMenus(root) {
    root = root || document;
    var cleanups = Array.from(
      root.querySelectorAll("details.arr-table-menu"),
    ).map(function (menu) {
  ...
  function mount(config) {
    ...
    if (!preferencesApi)
      throw new Error("InkDropOperationalTablePreferences is required");
    var table = config.table;
    var current = preferencesApi.apply(
      table,
      preferencesApi.load(config.storage),
    );
    var status =
      config.status || element("span", "inkdrop-table-preference-status");
    ...
    function update(patch, message) {
      current = preferencesApi.apply(
        table,
        preferencesApi.update(patch, config.storage),
      );
    ...
    var details = element(
      "details",
      "arr-table-menu inkdrop-table-preference-menu",
    );
    var summary = element("summary", "", "Options");
    summary.title =
      "Choose table columns, page size, thumbnails, and detail behavior";
    var panel = element(
      "div",
      "arr-table-menu-panel inkdrop-table-preference-panel",
    );
    ...
    if (columns.length) {
      ...
      columns.forEach(function (column) {
        ...
        checkbox.checked =
          ...
          current.columns.indexOf(column.key) >= 0;
        checkbox.disabled = column.required === true;
        if (column.required === true)
          checkbox.title = "This identity column is required.";
        checkbox.onchange = function () {
          var selected = Array.from(
            columnGroup.querySelectorAll("input[type=checkbox]:checked"),
          ).map(function (input) {
            return input.value;
          });
    ...
    thumbnails.onchange = function () {
      update(
        ...
        thumbnails.checked ? "Thumbnails shown." : "Thumbnails hidden.",
      );
    };
    ...
    var compactButton = element(
      "button",
      "inkdrop-table-view-choice",
      "Compact",
    );
    ...
    futureButton.title =
      "Card view is unavailable on operational pages; use the Series library for posters.";
    futureButton.setAttribute(
      "aria-label",
      "Cards unavailable: use the Series library for posters.",
    );

    function refreshViewButtons() {
      compactButton.setAttribute(
        "aria-pressed",
        current.density === "compact" ? "true" : "false",
      );
      detailedButton.setAttribute(
        "aria-pressed",
        current.density === "detailed" ? "true" : "false",
      );
    }
    refreshViewButtons();
    ...
    if (config.resetButton) {
      config.resetButton.onclick = function () {
        current = preferencesApi.apply(
          table,
          preferencesApi.reset(config.storage),
        );
