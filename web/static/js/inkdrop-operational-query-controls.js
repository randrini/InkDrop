(function (global) {
  ...
  function mount(config) {
    ...
    function announce(message) {
      ...
      if (typeof config.onChange === "function")
        config.onChange(Object.assign({}, state));
    }
    ...
    function invokeSort() {
      ...
      if (typeof config.onSort === "function")
        config.onSort(state.sort, state.direction);
      announce(
        "Sorted by " +
          (labelFor(config.sorts, state.sort) || "default order") +
          " " +
          state.direction +
          ".",
      );
    ...
    function invokeFilters() {
      ...
      if (typeof config.onFilter === "function")
        config.onFilter(Object.assign({}, state.filters));
    ...
    (config.sorts || []).forEach(function (item) {
      var unsupported =
        ...
        (config.sortMode === "client" && config.fullSetLoaded !== true);
      var reason =
        item.reason ||
        (unsupported
          ? "This sort requires server support or the complete result set."
          : "");
      ...
      sortMenu.panel
        ...
        .forEach(function (button) {
          ...
          if (item)
            button.setAttribute(
              "aria-checked",
              state.sort === item.key ? "true" : "false",
            );
          if (text === "Ascending")
            button.setAttribute(
              "aria-checked",
              state.direction === "asc" ? "true" : "false",
            );
          if (text === "Descending")
            button.setAttribute(
              "aria-checked",
              state.direction === "desc" ? "true" : "false",
            );
        });
      filterMenu.panel
        .querySelectorAll("[role=menuitemradio]")
        .forEach(function (button) {
          ...
          if (item)
            button.setAttribute(
              "aria-checked",
              state.filters[item.key] === true ? "true" : "false",
            );
        });
    }
    ...
    var cleanupMenus = controlsApi
      ? controlsApi.enhanceMenus(config.menuRoot || document)
      : function () {};
    ...
    function reset() {
      ...
      if (typeof config.onReset === "function")
        config.onReset(Object.assign({}, state));
  ...
  global.InkDropOperationalQueryControls = Object.freeze({
    mount: mount,
