(function (global) {
  ...
  function registryRow(state, type) {
    return (
      (state.registry || []).find((row) => String(row?.client_type || row?.client_id || "").toLowerCase() === type) ||
      null
    );
  }
  ...
  function showChooser(state, trigger) {
    const available = new Map(
      selectableRegistry(state).map((row) => [String(row.client_type || row.client_id).toLowerCase(), row]),
    );
    const body = shell(
      "Add Download Client",
    ...
    if (!available.size)
      form.append(el("p", "download-client-inline-error", "InkDrop has no download clients you can add or test yet."));
    ...
    body
      ...
      .forEach((button) => button.addEventListener("click", () => closeDialog(state)));
  ...
  function appendMappings(form, instance) {
    ...
    section.append(rows);
    const add = el("button", "secondary download-client-add-mapping", "Add Path Mapping");
    add.type = "button";
  ...
  function showEditor(state, instance, trigger, options) {
    ...
    if (Object.prototype.hasOwnProperty.call(fields, "username"))
      field(form, "Username", "username", { value: instance.username || "", autocomplete: "username" });
    ...
    if (
      ...
      ["qbittorrent", "sabnzbd", "transmission", "deluge", "nzbget", "utorrent", "rtorrent"].includes(type)
    )
    ...
    if (Object.prototype.hasOwnProperty.call(fields, "download_path") || type !== "sabnzbd")
    ...
    for (const media of ["comics", "manga", "ebooks"]) {
      field(routing, `${media[0].toUpperCase() + media.slice(1)} category`, `category:${media}`, {
        value: instance.categories?.[media] || "",
      });
      field(routing, `${media[0].toUpperCase() + media.slice(1)} path`, `download_path:${media}`, {
    ...
    if (type === "slskd") {
      ...
      for (const [key, label, help, min] of SLSKD_ADVANCED)
        field(grid, label, `setting:${key}`, { type: "number", min, value: instance.settings?.[key] ?? "", help });
    ...
    if (editing) {
      ...
      savedTest.addEventListener("click", async () => {
        ...
        try {
          const result = await api(`/api/download-clients/${encodeURIComponent(instance.id)}/test`, {
            method: "POST",
            body: {},
          });
          const tested = result.status?.result || {};
          error.hidden = false;
          error.dataset.tone = tested.ok ? "good" : tested.skipped ? "warn" : "bad";
    ...
    const save = el("button", "primary", editing ? "Save Changes" : "Add Client");
    save.type = "submit";
    footer.append(save);
    form.append(footer);
  ...
  function statusText(status) {
    ...
    if (status?.skipped || result.skipped)
      return { text: `Skipped · ${status?.reason || result.reason || "not ready"}`, tone: "warn" };
  ...
  async function testSaved(state, instance, button) {
    ...
    try {
      const payload = await api(`/api/download-clients/${encodeURIComponent(instance.id)}/test`, {
        method: "POST",
        body: {},
      });
  ...
  async function toggleEnabled(state, instance, button) {
    ...
    try {
      ...
      await load(state);
      setLive(state, `${instance.name} ${instance.enabled ? "disabled" : "enabled"}.`, "good");
    } catch (err) {
      setLive(state, errorText(err, `Could not ${instance.enabled ? "disable" : "enable"} ${instance.name}.`), "bad");
  ...
  function renderCards(state) {
    ...
    for (const instance of state.instances) {
      ...
      statusWrap.append(el("dt", "", "Health"), el("dd", `download-client-health ${status.tone}`, status.text));
      ...
      toggle.type = "button";
      toggle.addEventListener("click", () => toggleEnabled(state, instance, toggle));
      const remove = el("button", "danger", "Delete");
      remove.type = "button";
      remove.addEventListener("click", () => confirmDelete(state, instance, remove));
  ...
  function hideMigratedLegacyCards(state) {
    ...
    group?.querySelectorAll?.(".settings-card[data-provider-id]").forEach((card) => {
      ...
      card.dataset.downloadClientLegacyFallback = migrated ? "hidden-by-instance" : "visible";
    });
  }
  ...
  async function runTestAll(state) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 75; attempt += 1) {
        ...
        const payload = await api(`/api/download-clients/test-runs/${encodeURIComponent(runId)}`, {
          method: "GET",
          cache: "no-store",
        });
  ...
  async function load(state) {
    ...
    state.loading = api("/api/download-clients", { method: "GET", cache: "no-store" })
      ...
      .catch((err) => {
        state.cards.replaceChildren(
          el("div", "download-client-inline-error", errorText(err, "Could not load download clients.")),
        );
