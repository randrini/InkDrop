(() => {
  ...
  function stateLabel(payload) {
    if (state.actionError)
      return {
        label: state.actionError.action === "start" ? "Pass did not start" : "Pause did not apply",
        tone: "bad",
      };
  ...
  function renderPayload(payload) {
    ...
    if (!payload.enabled) {
      ...
      notice.append(
        element(
          "span",
          "",
          payload.config_action || "Enable missing recovery and restart the web and worker services.",
        ),
      );
      host.append(notice);
      ...
      host.append(
        element(
          ...
          payload.error || "Recovery counts are currently unknown. Refresh after state storage is available.",
        ),
      );
    }
    ...
    if (state.actionError) {
      ...
      failure.append(
        element("span", "mini", "Try again. If it keeps failing, check the InkDrop web log for the reason."),
      );
      host.append(failure);
    }
    ...
    for (const [key, label] of Object.entries(progressLabels)) {
      ...
      const tone =
        key === "needs_attention" && Number(value) ? "bad" : key === "complete" && Number(value) ? "good" : "";
    ...
    selection.append(
      metric(
        "Next-pass selection",
        payload.status_available ? Number(payload.selected_count || 0) : "Unknown",
      ...
      metric(
        ...
        payload.estimated_provider_calls?.detail || "Calculated after selection",
      ),
      metric(
        "New handoff limit",
    ...
    if (categoryRows.length) {
      ...
      categoryList.append(
        element(
          "span",
          "mini",
          payload.status_available ? "Nothing is picked for the next pass yet." : "Selection categories are unknown.",
        ),
      );
    }
    ...
    if (state.actionError) {
      ...
      live.textContent = "New recovery admission is paused. Active transfers and imports continue.";
