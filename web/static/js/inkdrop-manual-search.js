(function () {
  ...
  function api() {
    if (!window.InkDropApi || typeof window.InkDropApi.request !== "function") {
      throw Object.assign(new Error("InkDrop API client is unavailable."), {
  ...
  function attemptLabel(attempt) {
    const provider =
      attempt.provider_display_name ||
      attempt.provider_name ||
      attempt.provider_id ||
      "Provider";
    const child =
      attempt.child_source_name ||
      attempt.indexer_or_extension ||
      attempt.child_source_id ||
      "";
    return child && child.toLowerCase() !== text(provider).toLowerCase()
      ? `${provider} · ${child}`
      : provider;
  }
  ...
  function attemptGuidance(attempt) {
    ...
    const codes = Array.isArray(rows)
      ? rows
          .map((row) =>
            text(row?.failure_code || row?.reason_code).toLowerCase(),
          )
          .filter(Boolean)
      : [];
    const providerKey = text(attempt?.provider_id).toLowerCase();
    const isProwlarr =
      providerKey === "prowlarr" ||
      providerKey.startsWith("prowlarr_") ||
      providerKey.startsWith("prowlarr-");
    const genericMessages = {
      provider_credentials_unavailable:
        "Provider credentials unavailable. Check this provider's configuration in Settings.",
      provider_http_error:
        "Provider returned an HTTP error. Check its health in Settings > Download Sources.",
      provider_connection_failed:
        "InkDrop could not reach this provider. Check its configured address and container network.",
      provider_timeout:
        "Provider did not finish before the deadline. Check its health or adjust the timeout.",
      malformed_provider_response:
        "Provider returned an unreadable response. Check its logs and version.",
      ...
      provider_configuration_required:
        "Provider needs configuration in Settings.",
      provider_request_failed:
        "A provider request failed. Check its health and configuration.",
      unsupported_adapter:
        "This provider is not supported in this version of InkDrop.",
    ...
    const prowlarrMessages = {
      ...
      provider_http_error:
        "Prowlarr returned an HTTP error. Check the affected indexer in Prowlarr.",
      provider_connection_failed:
      ...
      malformed_provider_response:
        "Prowlarr returned an unreadable response. Check its logs and version.",
      provider_response_too_large:
      ...
      provider_configuration_required:
        "Prowlarr needs configuration in Settings > Indexers.",
      provider_request_failed:
        "A Prowlarr request failed. Check the provider-row details and Prowlarr indexer health.",
    };
    const messages = isProwlarr ? prowlarrMessages : genericMessages;
    const code =
      codes.find((value) => messages[value]) ||
      text(attempt?.error_code).toLowerCase();
    if (messages[code]) return messages[code];
    const stateName = text(attempt?.state || attempt?.status).toLowerCase();
    if (
      ["provider_failure", "provider_timeout", "timed_out"].includes(stateName)
    ) {
      return isProwlarr
  ...
  function decision(candidate) {
    ...
    if (raw === "source_unavailable")
      return { label: "Source Unavailable", tone: "bad", rank: 4 };
    if (candidate.accepted && candidate.pack_candidate)
      return { label: "Safe Pack", tone: "good", rank: 1 };
    if (candidate.accepted)
      return { label: "Safe Match", tone: "good", rank: 0 };
  ...
  function evidence(candidate) {
    const core =
      candidate.decision && typeof candidate.decision === "object"
        ...
        : {};
    const positive =
      core.positive_evidence || candidate.positive_evidence || [];
    const negative =
      core.negative_evidence ||
      candidate.rejection_explanations ||
  ...
  function sourceLabel(candidate) {
    const provider =
      ...
      candidate.provider_result_label ||
      candidate.provider_id ||
      "Unknown provider";
    const child =
      candidate.child_source_name || candidate.indexer_or_extension || "";
    return child && text(child).toLowerCase() !== text(provider).toLowerCase()
      ? `${provider} · ${child}`
      : provider;
  }
  ...
  function interpretedMatch(candidate) {
    return (
      [
        ...
        candidate.interpreted_unit_number
          ? `#${candidate.interpreted_unit_number}`
          : "",
      ]
        .filter(Boolean)
  ...
  function candidateMatches(candidate) {
    ...
    if (state.provider && text(candidate.provider_id) !== state.provider)
      return false;
    if (state.protocol && text(candidate.protocol) !== state.protocol)
      return false;
    if (state.language && text(candidate.language) !== state.language)
      return false;
    if (
      state.format &&
      text(candidate.format || candidate.file_archive_format) !== state.format
    )
      return false;
    if (state.pack === "pack" && !candidate.pack_candidate) return false;
    if (state.pack === "single" && candidate.pack_candidate) return false;
    if (state.assisted === "yes" && candidateDecision.label !== "Assisted")
      return false;
    if (state.assisted === "no" && candidateDecision.label === "Assisted")
      return false;
    if (
      state.minimumScore !== "" &&
      Number(candidate.match_score || 0) < Number(state.minimumScore)
    )
      return false;
    return true;
  }
  ...
  function sortedResults() {
    ...
    const get = (candidate) => {
      ...
      if (state.sort === "provider")
        return sourceLabel(candidate).toLowerCase();
  ...
  function optionValues(key) {
    return Array.from(
      new Set(state.results.map((item) => text(item[key])).filter(Boolean)),
    ).sort();
  }
  ...
  function providerProgressHtml() {
    if (!state.run && !state.attempts.length)
      return '<p class="manual-search-muted">Search has not started.</p>';
  ...
  function resultDetails(candidate) {
    ...
    const facts = [
      ...
      [
        "Query",
        candidate.query_evidence?.query || candidate.query || "Not supplied",
      ],
      ...
      [
        "Publisher / year",
        [
          candidate.interpreted_publisher || candidate.publisher,
          candidate.interpreted_year,
        ]
          .filter(Boolean)
          .join(" · "),
      ],
      [
        "Source health",
        candidate.source_health_snapshot?.status ||
          candidate.health_snapshot?.status,
      ],
  ...
  function resultRow(candidate) {
    ...
    const availability =
      candidate.seeders !== undefined && candidate.seeders !== null
        ...
        : candidate.remote_availability ||
          candidate.remote_queue_state ||
          "Unavailable";
    const pack = candidate.pack_candidate
      ...
      : "Single";
    let action = "";
    if (state.grabbed.has(candidateId))
      action = '<span class="manual-search-chip good">Sent to queue</span>';
      ...
      action =
        '<span class="manual-search-assisted">Manual retrieval required</span>';
    else
  ...
  function render() {
    ...
    const isRunning =
      !TERMINAL_STATES.has(text(runState).toLowerCase()) &&
      runState !== "not_started";
    const results = sortedResults();
    const allRejected =
      state.results.length > 0 && !state.results.some((item) => item.accepted);
    let resultBody = "";
    if (!state.run)
      resultBody =
    ...
    const preferred = element.querySelector(
      "[data-start-search], [data-close-manual-search]",
    );
  ...
  function showError(error, fallback) {
    ...
    if (
      ...
      message =
        "Your secure session needs to be refreshed. Sign in again, then retry this action.";
    }
  ...
  async function loadResults() {
    ...
    const data = await api().request(
      `/api/manual-search/runs/${encodeURIComponent(runId)}/results?${query}`,
    );
  ...
  async function refreshRun({ poll = false } = {}) {
    ...
    try {
      const [runData, diagnosticData] = await Promise.all([
        ...
        api()
          .request(
            `/api/manual-search/runs/${encodeURIComponent(runId)}/diagnostics`,
          )
      ...
      const diagnosticAttempts = Array.isArray(diagnosticData.provider_attempts)
        ...
        : [];
      if (diagnosticAttempts.length || !Array.isArray(state.attempts))
        state.attempts = diagnosticAttempts;
      ...
      if (!poll) showError(error, "Manual Search could not be refreshed.");
        ...
        showError(
          error,
          "Manual Search stopped updating. Use Refresh to try again.",
        );
      }
    }
  }
  ...
  async function startSearch(providers) {
    ...
    try {
      ...
      const data = await api().request("/api/manual-search/runs", {
        method: "POST",
        body,
      });
      state.run = {
      ...
      state.attempts = (data.providers || []).map((provider) =>
        typeof provider === "string"
          ? { provider_id: provider, state: "planned" }
          : provider,
      );
      state.grabbed.clear();
  ...
  async function cancelSearch() {
    ...
    try {
      ...
      await api().request(
        `/api/manual-search/runs/${encodeURIComponent(runId)}/cancel`,
        { method: "POST", body: {} },
      );
      await refreshRun();
    } catch (error) {
  ...
  async function grab(candidateId, { forceRejected = false } = {}) {
    const candidate = state.results.find(
      (item) => text(item.candidate_id) === text(candidateId),
    );
    if (
      !candidate ||
      state.busy ||
      (forceRejected ? !forceGrabAllowed(candidate) : !grabAllowed(candidate))
    )
      return;
    const proof = evidence(candidate);
    if (forceRejected) {
      ...
      const trigger = root()?.querySelector(
        `[data-force-grab="${escapeHtml(candidateId)}"]`,
      );
      if (trigger) {
    ...
    const warning =
      ...
      ["possible", "possible_match"].includes(
        text(candidate.decision?.decision).toLowerCase(),
      );
    if (warning && !forceRejected) {
      ...
      // TODO: Replace window.confirm with inkdropConfirmModal in a future refactor.
      const trigger = root()?.querySelector(
        `[data-grab="${escapeHtml(candidateId)}"]`,
      );
      if (trigger) {
        trigger.setAttribute("aria-busy", "true");
        trigger.disabled = true;
    ...
    try {
      const body = {
        ...
        override_hard_limit:
          forceRejected && proof.negative.includes("pack_hard_limit_exceeded"),
        force_rejected: forceRejected,
        confirm_rejected_risk: forceRejected,
      };
      const data = await api().request(
        `/api/manual-search/candidates/${encodeURIComponent(candidateId)}/grab`,
        { method: "POST", body },
      );
  ...
  function bind(element) {
    ...
    element
      .querySelector("[data-start-search]")
      ?.addEventListener("click", () => startSearch([]));
    element
      .querySelector("[data-search-selected]")
      ?.addEventListener("click", () => {
        const providers = Array.from(
          element.querySelectorAll("[data-provider-select]:checked"),
        ).map((input) => input.value);
        if (providers.length) startSearch(providers);
      });
    element
      .querySelector("[data-refresh-run]")
      ?.addEventListener("click", () => refreshRun());
    element
      .querySelector("[data-cancel-run]")
      ?.addEventListener("click", cancelSearch);
    element
      .querySelector("[data-include-rejected]")
      ?.addEventListener("change", async (event) => {
        state.includeRejected = event.target.checked;
        if (state.run) {
          await loadResults().catch((error) =>
            showError(error, "Rejected candidates could not be loaded."),
          );
        }
        render();
      });
    ...
    element
      ...
      .forEach((button) =>
        button.addEventListener("click", () => grab(button.dataset.grab)),
      );
    element
      .querySelectorAll("[data-force-grab]")
      .forEach((button) =>
        button.addEventListener("click", () =>
          grab(button.dataset.forceGrab, { forceRejected: true }),
        ),
      );
  }
  ...
  function open(context = {}) {
    if (!(context.issue_id || context.unit_id)) {
      window.dispatchEvent(
        new CustomEvent("inkdrop:manual-search-unavailable", {
          detail: { reason: "canonical_unit_id_required" },
        }),
      );
      return false;
    }
    ...
    if (url.searchParams.get(ROUTE_KEY) !== "1") {
      ...
      window.history.pushState(
        { ...(window.history.state || {}), inkdropManualSearch: true },
        "",
        url,
      );
  ...
  function close({ fromPopState = false } = {}) {
    ...
    if (
      !fromPopState &&
      state.pushedRoute &&
      new URL(window.location.href).searchParams.get(ROUTE_KEY) === "1"
    ) {
