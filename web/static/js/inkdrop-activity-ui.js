(function (global) {
  ...
  function formatSuccessfulAction(value) {
    ...
    const kind = humanize(first(value, ["kind", "action", "type"]) || "Successful action");
    const status = humanize(first(value, ["status", "result"]));
    const at = first(value, ["at", "completed_at", "created_at"]);
    return [kind, status, at ? formatRelative(at) : ""].filter(Boolean).join(" · ");
  }

  function apiGet(path) {
    if (global.InkDropApi && typeof global.InkDropApi.request === "function") {
      return global.InkDropApi.request(path, { method: "GET", cache: "no-store" });
    }
    return fetch(path, {
      method: "GET",
  ...
  function stageLabel(row) {
    ...
    const raw = normalize(first(row, ["stage", "state", "queue_state", "transfer_state"]));
    ...
    if (raw.includes("client_queued") || raw.includes("queued_remote")) return `Queued in ${sourceClient(row)}`;
  ...
  function progressModel(row) {
    ...
    if (client.includes("slskd") && (raw.includes("remote") || raw.includes("queued"))) {
      return { kind: "stage", label: "Remote queue", detail: "No trusted percentage available" };
    }
    ...
    const speed = number(first(row, ["speed_bytes_per_second", "rate_bytes_per_second", "download_rate"]));
    const parts = [];
    if (done != null && total) parts.push(`${formatBytes(done)} / ${formatBytes(total)}`);
    if (speed != null) parts.push(`${formatBytes(speed)}/s`);
    if (kind === "determinate" || pct != null) {
      const displayPercent = Math.round(Math.max(0, Math.min(100, pct || 0)) * 10) / 10;
      return { kind: "determinate", percent: displayPercent, detail: parts.join(" · ") };
    }
    return { kind: "stage", label: stageLabel(row), detail: parts.join(" · ") || "Indeterminate" };
  }
  ...
  function etaAge(row) {
    ...
    const updated = first(row, ["last_updated_at", "last_update", "updated_at", "last_seen_at"]);
    if (updated) return formatRelative(updated);
    return "";
  }

  function nextText(row) {
    return scrub(first(row, ["next_action", "next", "next_step", "action_summary"]) || "No action reported");
  }

  function activityId(row, index) {
    return text(first(row, ["activity_id", "id", "queue_id", "row_id"]) || `activity-${index}`);
  }
