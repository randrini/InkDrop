(function (global) {
  ...
  function stateLabel(rawState) {
    ...
    if (state.indexOf("fail") >= 0 || state.indexOf("error") >= 0 || state.indexOf("blocked") >= 0)
  ...
  function normalizeContract(row, nowSeconds) {
    ...
    return {
      ...
      detail: String(
        first([transfer.next_step, transfer.stalled_reason, transfer.client_error, transfer.native_state]) || "",
      ).trim(),
  ...
  function normalize(row, nowSeconds) {
    ...
    var progressScope = String(
      first([row.progress_scope, task.progress_scope, row.telemetry_scope, task.telemetry_scope]) || "",
    ).toLowerCase();
    ...
    var completedBytes = finite(
      first([task.completed_bytes, row.completed_bytes, task.downloaded_bytes, row.downloaded_bytes]),
    );
    ...
    var detail = String(
      first([task.stage_detail, row.stage_detail, row.next_action, row.activity_summary]) || "",
    ).trim();
    var parts = [];
    if (progress !== null) parts.push(Math.round(progress) + "%");
