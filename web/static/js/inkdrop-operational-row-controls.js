(function (global) {
  ...
  function mount(config) {
    ...
    var disclosure = config.disclosure || element("button", "inkdrop-row-disclosure", "Details");
    disclosure.type = "button";
    disclosure.setAttribute("aria-expanded", details && details.open ? "true" : "false");
    if (details && details.id) disclosure.setAttribute("aria-controls", details.id);
    ...
    function setOpen(open) {
      ...
      disclosure.title = open ? "Hide operational details" : "Show operational details";
    ...
    if (!(config.actions || []).length) {
      var unavailable = element("span", "inkdrop-row-action-empty", "No actions available");
      unavailable.title = "This row has no available actions.";
      panel.append(unavailable);
    }

    if (config.disclosureHost) config.disclosureHost.replaceChildren(disclosure);
    ...
    var cleanup = controlsApi ? controlsApi.enhanceMenu(actionMenu, config.menuRoot || document) : function () {};

    return Object.freeze({ setOpen: setOpen, status: status, destroy: cleanup });
  }
